...
def new_session_message(function):
    @wraps(function)
    def wrap(request, *args, **kwargs):
        if (request.session.get("first_session") is None and
                Games.objects.filter(registrations_closed=False).count() != 0):
            current_site = get_current_site(request)
            messages.info(
                request,
                "Please check the event schedule <a href='https://miku.anime-thighs-are-the.best/miku-chan/ZX0IZLuL' target='_blank'>from here</a>",
            )
            messages.info(
                request,
