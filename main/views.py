    ...
    if request.method == 'POST':
        ...
        for i in request.POST.dict():
            if 'userid' in i:
                tuple_3 = i.split(' ')
                group_id = tuple_3[1]
                if settings.AUTH_USER_MODEL.objects.filter(unique_id=request.POST.dict()[i]).exists():
                    users_list.append(settings.AUTH_USER_MODEL.objects.filter(unique_id=request.POST.dict()[i]))
                else:
        ...
        if len(users_list) > 0:
            ...
            for i in users_list:
                groups.users.add(i)
            message.success(request, f'Successfully added {len(users_list)} user(s) to the group!')
            parameters.update({'message_group_id': group_id})
        if len(inavlid_users_list) > 0:
            message.error(request, f'Unable to add {len(users_list)} user(s) to the group!')
            message.error(request, 'Make sure that they have registered themselves first ¯\_(ツ)_/¯')
            parameters.update({'message_group_id': group_id})
    return render(
        request,
        'groups.html',
